...
function calcularOpcion1() {
    ...
    document.getElementById("resultadoEncuentro1").innerText =
        "Resultados:\n" +
        `Tiempo (dt): ${dt.toFixed(3)} ${tiempoUnidad}\n` +
        `Desplazamiento(Dx): ${x.toFixed(3)} ${distanciaUnidad}`;
}
function calcularEncuentro2() {
    // Entradas del usuario
    let vAzul = parseFloat(document.getElementById("velocidadAzul2").value.replace(",", "."));
    let unidadVAzul = document.getElementById("unidadVelocidadAzul2").value;

    let vRoja = parseFloat(document.getElementById("velocidadRoja2").value.replace(",", "."));
    let unidadVRoja = document.getElementById("unidadVelocidadRoja2").value;

    let distancia = parseFloat(document.getElementById("distancia2").value.replace(",", "."));
    let unidadDistancia = document.getElementById("unidadDistancia2").value;

    if (isNaN(vAzul) || isNaN(vRoja) || isNaN(distancia)) {
        document.getElementById("resultadoEncuentro2").innerText = "Por favor completa todos los datos.";
        return;
    }

    // Unificar unidades si hay mezcla, basándonos en la que predomina
    const velocidades = [unidadVAzul, unidadVRoja];
    const msCount = velocidades.filter(u => u === "ms").length;
    const kmhCount = velocidades.filter(u => u === "kmh").length;

    let unidadVelocidadFinal = unidadVAzul; // Por defecto, tomar la de la azul
    if (msCount > kmhCount) {
        unidadVelocidadFinal = "ms";
    } else if (kmhCount > msCount) {
        unidadVelocidadFinal = "kmh";
    }

    let unidadDistanciaFinal = unidadDistancia;
    if (unidadVelocidadFinal === "ms" && unidadDistancia === "km") {
        distancia *= 1000;
        unidadDistanciaFinal = "m";
    } else if (unidadVelocidadFinal === "kmh" && unidadDistancia === "m") {
        distancia /= 1000;
        unidadDistanciaFinal = "km";
    }

    // Convertir velocidades a la unidad final
    if (unidadVAzul !== unidadVelocidadFinal) {
        vAzul = (unidadVelocidadFinal === "ms") ? vAzul / 3.6 : vAzul * 3.6;
    }
    if (unidadVRoja !== unidadVelocidadFinal) {
        vRoja = (unidadVelocidadFinal === "ms") ? vRoja / 3.6 : vRoja * 3.6;
    }

    // Validar que azul sea más rápida
    if (vAzul <= vRoja) {
        document.getElementById("resultadoEncuentro2").innerText =
            "No hay encuentro porque la esfera azul no alcanza a la roja.";
        return;
    }

    // Calcular en la unidad predominante
    const tiempo = distancia / (vAzul - vRoja); // Tiempo en segundos o horas
    const desplazamiento = vRoja * tiempo;     // Distancia en m o km

    // Mostrar resultados con la unidad predominante
    const tiempoUnidad = (unidadVelocidadFinal === "kmh") ? "horas" : "segundos";
    const distanciaUnidad = (unidadDistanciaFinal === "km") ? "kilómetros" : "metros";

    document.getElementById("resultadoEncuentro2").innerText =
        `Resultados:\n` +
        `Tiempo hasta el encuentro: ${tiempo.toFixed(1)} ${tiempoUnidad}\n` +
        `Desplazamiento desde la posición inicial de la esfera roja (x): ${desplazamiento.toFixed(1)} ${distanciaUnidad}`;
}
